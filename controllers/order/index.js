...
const { countDiscount } = require("../../utils/countSellPrice");
...
exports.calculateOrder = async (req, res, next) => {
	...
	try {
		...
		processingCarts?.map?.((cart) => {
			...
			sellPrice += cart?.quantity * cart?.variant?.product?.sellPrice || 0;
			discountPrice +=
				cart?.quantity * cart?.variant?.product?.discount?.amount || 0;
		});

		const location = await Address.findById(shipping);

		if (!location) throw new ErrorResponse("No shipping location found", 404);
		if (!location?.type)
			throw new ErrorResponse("No shipping type found in location", 404);

		shippingFee = deliveryOptions[location.type];
			...
			discountPrice +=
				cart?.quantity *
					countDiscount(
						...
						cart?.variant?.product?.discount?.amount
					) || 0;
		});
	...
	try {
		...
		processingCarts?.map?.(async (cart) => {
			...
			await OrderLine.create({
				...
				discount: cart.variant?.product?.discount?._id,
				discountPrice: cart.variant?.product?.discount?.amount || 0,
			});
...
const cancelOrderLine = async (order, orderlinesIds, status) => {
	...
	orderLines?.map?.(async (orderline) => {
		...
		order.discount -= orderline.discountPrice;
