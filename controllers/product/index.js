...
exports.getByDiscountId = async (req, res, next) => {
	// Get Values
	const { category_id } = req.params;
	...
	const body = await DiscountJunction.paginate(
		{
			...(req.search && {
				$or: [
					...queryObjectBuilder(
			...
			$and: [
				{
					...flatSubquery("discount._id", category_id),
				},
				{
					discount: { $ne: null },
				},
			...
			populate: [
				{
					path: "product",
					select:
						"titleEn titleBn category subcategory store slug buyPrice sellPrice price image isActive",
					populate: [
						{
							path: "category",
							select: "titleEn titleBn icon isActive slug",
	...
	try {
		res.status(200).json({
			...
			message: "Product list fetched successfully",
			...body,
			data: body.data?.flatMap((x) => x.product),
		});
...
exports.getByOfferId = async (req, res, next) => {
	// Get Values
	const { category_id } = req.params;
	...
	const body = await DiscountJunction.paginate(
		{
			...
			...flatSubquery("product.category.isActive", true),
			...flatSubquery("product.subcategory.isActive", true),
			...flatSubquery("product.store.isActive", true),
			$and: [
				{
					...flatSubquery("discount._id", category_id),
				},
